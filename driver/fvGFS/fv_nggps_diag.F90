...
! </table>
 ...
 integer :: id_ug80m, kstt_ug80m, kend_ug80m
 integer :: id_vg80m, kstt_vg80m, kend_vg80m
 integer :: id_vort, kstt_vort, kend_vort
 integer :: nlevs
...
!-------------------
       ...
       id_vort = register_diag_field ( trim(file_name), 'rel_vort', axes(1:3), Time,        &
            'relative vorticity', '1/s', missing_value=missing_value )
       if (id_vort>0) then
          kstt_vort = nlevs+1; kend_vort= nlevs+npzo
          nlevs = nlevs + npzo
       endif
...
!
       ...
       id_ug80m = register_diag_field ( trim(file_name), 'ug80m',axes(1:2),Time,       &
          'u at 80m AGL', 'm/s', missing_value=missing_value )
       if( id_ug80m > 0) then
          kstt_ug80m = nlevs+1; kend_ug80m = nlevs+1
          nlevs = nlevs + 1
       endif
       id_vg80m = register_diag_field ( trim(file_name), 'vg80m',axes(1:2), Time,&
          'v at 80m AGL', 'm/s', missing_value=missing_value )
       if( id_vg80m > 0) then
          kstt_vg80m = nlevs+1; kend_vg80m = nlevs+1
          nlevs = nlevs + 1
       endif
...
!------------------------------------
 ...
 subroutine fv_nggps_diag(Atm, zvir, Time)
    ...
    real, dimension(:,:),allocatable :: ustm,vstm,srh01,srh03,hailcast_dhail_max,a2
    n = 1
    ngc = Atm(n)%ng
    ptop = Atm(n)%ak(1)
    allmax = -20.
    ...
    !---- RELATIVE VORTICITY
    call get_vorticity(isco, ieco, jsco, jeco, isdo, iedo, jsdo, jedo, npzo,Atm(n)%u,Atm(n)%v, wk, &
               Atm(n)%gridstruct%dx,Atm(n)%gridstruct%dy,Atm(n)%gridstruct%rarea)
    if(id_vort > 0) call store_data(id_vort,wk, Time, kstt_vort,kend_vort)
...
!       'rdgas=',rdgas,'grav=',grav,'stndrd_atmos_lapse=',stndrd_atmos_lapse,rdgas/grav*stndrd_atmos_lapse
    ...
    if ( id_ustm > 0 .or. id_vstm > 0 .or. id_srh01 > 0 .or. id_srh03 > 0) then
      ...
      allocate ( a2(isco:ieco,jsco:jeco) )
      if ( id_ug80m>0 ) then
            call interpolate_z_new(isco, ieco, jsco, jeco, ngc, npzo, zvir, sphum, &
                 Atm(n)%delz, Atm(n)%q, Atm(n)%flagstruct%hydrostatic, Atm(n)%pt, &
                 Atm(n)%peln, grav, 0.,80.,Atm(n)%ua,a2)
!            call interpolate_z(isco, ieco, jsco, jeco, npzo,
!            80.,wz,Atm(n)%ua(isco:ieco,jsco:jeco,:), a2)
            call store_data(id_ug80m, a2, Time,kstt_ug80m,kend_ug80m)
       endif
       if ( id_vg80m>0 ) then
            call interpolate_z_new(isco, ieco, jsco, jeco, ngc, npzo, zvir,sphum, &
                 Atm(n)%delz, Atm(n)%q, Atm(n)%flagstruct%hydrostatic,Atm(n)%pt, &
                 Atm(n)%peln, grav, 0.,80.,Atm(n)%va,a2)
!            call interpolate_z(isco, ieco, jsco, jeco, npzo, 80.,wz,Atm(n)%va(isco:ieco,jsco:jeco,:), a2)
            call store_data(id_vg80m, a2, Time,kstt_vg80m,kend_vg80m)
       endif
    deallocate(a2)
...
!
   ...
   if( id_vort > 0) then
     call find_outputname(trim(file_name),'rel_vort',output_name)
     call add_field_to_bundle(trim(output_name),'relative vorticity', '1/s',"time: point",   &
          axes(1:3), fcst_grid, kstt_vort,kend_vort, dyn_bundle, output_file,rcd=rc)
     if(rc==0)  num_field_dyn=num_field_dyn+1
   endif
...
!     if(mpp_pe()==mpp_root_pe())print *,'reflectivity, output name=',trim(output_name)
   ...
   if(id_ug80m > 0) then
     call find_outputname(trim(file_name),'ug80m',output_name)
     if(mpp_pe()==mpp_root_pe())print *,'80-m u wind speed,output name=',trim(output_name)
     call add_field_to_bundle(trim(output_name),'80-m u wind speed', 'm/s', "time: point",   &
          axes(1:2), fcst_grid, kstt_ug80m,kend_ug80m, dyn_bundle,output_file, rcd=rc)
     if(rc==0)  num_field_dyn=num_field_dyn+1
   endif
   if(id_vg80m > 0) then
     call find_outputname(trim(file_name),'vg80m',output_name)
     if(mpp_pe()==mpp_root_pe())print *,'80-m v wind speed,output name=',trim(output_name)
     call add_field_to_bundle(trim(output_name),'80-m v wind speed', 'm/s',"time: point",   &
          axes(1:2), fcst_grid, kstt_vg80m,kend_vg80m, dyn_bundle,output_file,rcd=rc)
     if(rc==0)  num_field_dyn=num_field_dyn+1
   endif
